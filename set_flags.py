...
def setZero(result):
    if int(result,16) == 0:
        regi.flag.update({"Z":1})
    else:
        regi.flag.update({"Z":0})


def setCarry(result):
    if int(result, 16) > 255:
        regi.flag.update({"CY":1})
        return format((int(result, 16) - 256), '0x')
    if int(result, 16) < 0:
        regi.flag.update({"CY": 1})
        return format((int(result, 16) + 256), '0x')
    else:
        regi.flag.update({"CY": 0})


def setParity(result):
    parity = result.count("1")
    if parity%2 == 0:
        regi.flag.update({"P":1})
    else:
        regi.flag.update({"P": 0})

def setAC(res1, res2):
    if res1+res2 > 15:
        regi.flag.update({"AC":1})
    else:
        regi.flag.update({"AC":0})


def setSign(result):
    if result[0] == "1":
        regi.flag.update({"S": 1})
    else:
        regi.flag.update({"S": 0})
